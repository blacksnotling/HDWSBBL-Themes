<?php get_header(); ?>
	<?php if (have_posts()) : ?>
		...
		<h2><?php printf( __( 'Tag Archives: %s', 'crownstar' ), '<span>' . single_tag_title( '', false ) . '</span>' ); ?></h2>
		...
		<?php while (have_posts()) : the_post(); ?>
			<div class="entry">
				<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<?php wp_link_pages( array( 'before' => '<div class="page-link">' . __( 'Pages:', 'crownstar' ), 'after' => '</div>' ) ); ?>
		...
		<?php endwhile; else: ?>
					<p><?php _e('Sorry, no posts have been posted under this tag.', 'crownstar'); ?></p>
		<?php endif; ?>
